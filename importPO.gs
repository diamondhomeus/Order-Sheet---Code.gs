...
function updateOrderList(){
  ...
  try{
    ...
    var response,responseObj,orders;
    while(fetchOrder){
      ...
      responseObj = XML_to_JSON(response);
      if(responseObj.ListOrdersResponse != undefined && responseObj.ListOrdersResponse.ListOrdersResult != undefined){
        responseObj = responseObj.ListOrdersResponse.ListOrdersResult;
        if(responseObj.Orders != undefined && responseObj.Orders.Order != undefined){
          orders = responseObj.Orders.Order;
        }
      }
      else if(responseObj.ListOrdersByNextTokenResponse != undefined && responseObj.ListOrdersByNextTokenResponse.ListOrdersByNextTokenResult != undefined){
        responseObj = responseObj.ListOrdersByNextTokenResponse.ListOrdersByNextTokenResult;
        if(responseObj.Orders != undefined && responseObj.Orders.Order != undefined){
          orders = responseObj.Orders.Order;
        }
      }
      ...
      if(orders.length > 0){
        ...
        if(responseObj.NextToken != undefined){
          var nextToken = responseObj.NextToken.Text;
          api_url = getApiURL("",nextToken);
          fetchOrder = true;
        }        
      }
    }
    
   var lr = last_row(sheetInv,1);
...
function updateBlankOrderData(){
  ...
  statussheet.getRange(5, 2).setValue(newdate);
...
function updateBlankOrderRows(api_url){
  ...
  try{
    ...
    if(orders){
      ...
      sheetInv.getRange(3,2,buyerEmailData.length).setValues(buyerEmailData);
      sheetInv.getRange(3,3,buyerNameData.length).setValues(buyerNameData);
      sheetInv.getRange(3,4,phoneData.length).setValues(phoneData);
    }
  }
  catch(e){
    Logger.log(e);
...
function getApiURL(orderIDs,nextToken){
  var now = new Date();
  ...
  if(nextToken){
    payload = {};
    payload["AWSAccessKeyId"]=awsAccessKeyId;
    payload["Action"] = "ListOrdersByNextToken";  
